...
<div class="p-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold tracking-tight">Ajouter une section</h1>
        <p class="text-zinc-500">Créez une nouvelle section pour organiser vos classes.</p>
    </div>
    ...
    <div class="max-w-2xl rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
        <form action="{{ route('sections.store') }}" method="POST" class="space-y-4">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required placeholder="ex: Primaire, Secondaire, Humanités">
                @error('name') <p class="text-xs text-red-500">{{ $message }}</p> @enderror
            </div>

            <div class="space-y-2">
                <label for="code" class="text-sm font-medium">Code de la section</label>
                <input type="text" name="code" id="code" value="{{ old('code') }}" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required placeholder="ex: PRI, SEC, HUM">
            ...
            <div class="space-y-2">
                ...
                <select name="education_level" id="education_level" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required>
                    <option value="preschool" {{ old('education_level') == 'preschool' ? 'selected' : '' }}>Maternelle</option>
                    <option value="primary" {{ old('education_level') == 'primary' ? 'selected' : '' }}>Primaire</option>
                    <option value="secondary" {{ old('education_level', 'secondary') == 'secondary' ? 'selected' : '' }}>Secondaire</option>
                    <option value="vocational" {{ old('education_level') == 'vocational' ? 'selected' : '' }}>Professionnel</option>
                    <option value="university" {{ old('education_level') == 'university' ? 'selected' : '' }}>Enseignement Supérieur / Université</option>
                </select>
