...
<div class="p-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold tracking-tight">Modifier les frais</h1>
        <p class="text-zinc-500">Ajustez le montant ou l'échéance.</p>
    </div>

    <div class="max-w-2xl rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
        <form action="{{ route('fees.update', $fee) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <select name="registration_id" id="registration_id" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required>
                    @foreach($registrations as $registration)
                        <option value="{{ $registration->id }}" {{ old('registration_id', $fee->registration_id) == $registration->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $fee->title ?? '') }}" placeholder="Ex: Mensualité Septembre" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required>
                ...
                <div class="space-y-2">
                    ...
                    <select name="type" id="type" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required>
                        <option value="tuition" {{ old('type', $fee->type ?? 'tuition') == 'tuition' ? 'selected' : '' }}>Frais de scolarité (Mensuel)</option>
                        <option value="registration" {{ old('type', $fee->type ?? 'tuition') == 'registration' ? 'selected' : '' }}>Frais d'inscription</option>
                        <option value="exam" {{ old('type', $fee->type ?? 'tuition') == 'exam' ? 'selected' : '' }}>Frais d'examen</option>
                        <option value="other" {{ old('type', $fee->type ?? 'tuition') == 'other' ? 'selected' : '' }}>Autre</option>
                    </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-2">
                    ...
                    <input type="number" step="0.01" name="amount_due" id="amount_due" value="{{ old('amount_due', $fee->amount_due) }}" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="currency" id="currency" value="{{ old('currency', $fee->currency) }}" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800" required>
            ...
            <div class="space-y-2">
                ...
                <input type="date" name="due_date" id="due_date" value="{{ old('due_date', $fee->due_date) }}" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800">
            ...
            <div class="space-y-2">
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full rounded-md border border-zinc-200 bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-950 dark:border-zinc-800">{{ old('notes', $fee->notes) }}</textarea>
